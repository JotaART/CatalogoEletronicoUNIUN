...
public class CarrinhoTableModel extends AbstractTableModel  {
    ...
    private final String[] colunas = {"Codigo","Valor Unit.","Qtd","Valor Tot.","del"};
    
    public Double getDesconto(Double precoDesconto) throws FileNotFoundException {      
        return precoDesconto;
    }

    public CarrinhoTableModel(List<Produtosbean> dados) {
        this.dados = dados;
    }
    ...
    public Object getValueAt(int linha, int coluna) {
        ...
        switch (coluna) {
            ...
            case 3: {
                try {
                    return (dados.get(linha).getPreco() * getDesconto(precoDesconto) * dados.get(linha).getQtd());
                } catch (FileNotFoundException ex) {
                    Logger.getLogger(CarrinhoTableModel.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    public Object getValueAtColuna(int linha, int coluna) {
        switch(linha){
            case 0:
                return dados.get(coluna).getPrecototal();
        }
        return null;
    }
